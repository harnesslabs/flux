...
- Plane and sphere share the library evolution mechanics in `flux.evolution`; mesh construction, solve runtimes, and exact solutions stay local to each system/runtime.
- The example-common runner still owns snapshot cadence, progress, and reference-study plumbing, so the example files are mostly problem setup plus `Evolution(...).config().dt(...).init(...)`.
