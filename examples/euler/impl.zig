...
const State2D = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, mesh: *const Mesh2D) !State2D {
    ...
    pub fn deinit(self: *State2D, allocator: std.mem.Allocator) void {
...
fn buildEdgeAdjacency(allocator: std.mem.Allocator, mesh: *const Mesh2D) ![]State2D.EdgeAdjacency {
...
fn refreshDerivedFields(allocator: std.mem.Allocator, state: *State2D) !void {
    try recoverStreamFunction(allocator, state);
    reconstructFaceVelocity(state);
}

fn recoverStreamFunction(allocator: std.mem.Allocator, state: *State2D) !void {
...
const State3D = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, mesh: *const Mesh3D) !State3D {
    ...
    pub fn deinit(self: *State3D, allocator: std.mem.Allocator) void {
...
fn writeSnapshot(allocator: std.mem.Allocator, writer: anytype, state: *const State3D) !void {
...
pub fn step(comptime topological_dimension: u8, allocator: std.mem.Allocator, state: *State(topological_dimension), dt: f64) !void {
    switch (topological_dimension) {
        2 => try step2D(allocator, state, dt),
        3 => try step3D(allocator, state, dt),
        else => unreachable,
    }
}
...
pub fn seedReferenceMode(comptime topological_dimension: u8, allocator: std.mem.Allocator, state: *State(topological_dimension)) !void {
    ...
    if (topological_dimension == 3) {
        return seedReferenceMode3D(allocator, state);
    }
    unreachable;
}
...
pub fn conservedQuantity(comptime topological_dimension: u8, allocator: std.mem.Allocator, state: *const State(topological_dimension)) !f64 {
    ...
    if (topological_dimension == 3) {
        return computeHelicity3D(allocator, state);
    }
    unreachable;
}
