...
//!
//! Extracted from `examples/maxwell_2d/main.zig` so all simulations render
//! progress, ETA, and steps-per-second using the same widget. The bar is
//! generic over the writer type so it works with whatever stderr handle the
//! caller produces.

const std = @import("std");

pub fn Progress(comptime Writer: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn init(writer: Writer, total: u32) Self {
            return .{
                .writer = writer,
                .total = total,
