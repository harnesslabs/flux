...
const root_flux = @import("flux");

const maxwell_2d = struct {
    ...
    //!
    //! Update functions are standalone (not methods on `StateForMesh`) to keep
    ...
    fn hooks(comptime MeshType: type) type {
        return struct {
            ...
            pub fn ampereStep(allocator: local_std.mem.Allocator, state: anytype, dt: f64) !void {
                ...
                for (state.E.values, d_star_B.values, edge_volumes, dual_edge_volumes, state.J.values) |*e, dsb, volume, dual_volume, j| {
                    local_std.debug.assert(dual_volume > 0.0);
                    e.* += dt * ((volume / dual_volume) * dsb - j);
                }

                comptime {
                    const BType = @TypeOf(state.B);
                    local_std.debug.assert(BType.degree == 2 and BType.duality == cochain.Primal);
                    local_std.debug.assert(@TypeOf(state.E).degree == 1 and @TypeOf(state.E).duality == cochain.Primal);
                }
            }
        };
    }

    pub fn StateForMesh(comptime MeshType: type) type {
        comptime {
    ...
    pub fn MaxwellSystem(comptime MeshType: type) type {
        const StateType = StateForMesh(MeshType);

        return struct {
            pub const StateForMesh = StateType;

            /// Faraday half-step: B^{n+1/2} = B^{n-1/2} − dt · dE^n.
            pub fn first_half_step(allocator: local_std.mem.Allocator, state: *StateType, dt: f64) !void {
                try faraday_step(allocator, state, dt);
            }

            /// Ampere half-step: E^{n+1} = E^n + dt · (★₁⁻¹ d ★₂ B^{n+1/2} − J).
            /// Also advances the discrete timestep counter.
            pub fn second_half_step(allocator: local_std.mem.Allocator, state: *StateType, dt: f64) !void {
    ...
    pub fn PointDipole(comptime MeshType: type) type {
        return struct {
            ...
            pub fn init(mesh: *const MeshType, frequency: f64, amplitude: f64, position: [MeshType.embedding_dimension]f64) Self {
                ...
                var best_edge: u32 = 0;
                var best_distance_squared: f64 = local_std.math.inf(f64);
            ...
            pub fn apply(self: Self, J: anytype, t: f64) void {
                ...
                J.values[self.edge_index] = (self.amplitude / self.edge_length) *
                    @sin(2.0 * local_std.math.pi * self.frequency * t);
            }
        };
    }
    ...
    const DriverState = StateForMesh(Mesh2D);
    ...
    pub fn runDriver(allocator: local_std.mem.Allocator, config: Config2D) !void {
        var stderr_buffer: [1024]u8 = undefined;
        var stderr_writer = local_std.fs.File.stderr().writer(&stderr_buffer);
    ...
    fn runDipole(allocator: local_std.mem.Allocator, config: Config2D, writer: anytype) !void {
    ...
    fn runCavity(allocator: local_std.mem.Allocator, config: Config2D, writer: anytype) !void {
        const h = config.spacing();
        const time_step = config.dt();
        const field_period = 2.0 * config.domain;
        const omega = local_std.math.pi / config.domain;
    ...
    pub fn Runner(comptime MeshType: type) type {
        return struct {
            const Self = @This();
            const MState = StateForMesh(MeshType);
            ...
            pub fn run(
                ...
                local_std.debug.assert(config.steps > 0);
                local_std.debug.assert(config.dt > 0);
            ...
            fn writePvd(
                ...
                const pvd_name = local_std.fmt.bufPrint(&pvd_buf, "{s}.pvd", .{base_name}) catch
    ...
    const MaxwellState = StateForMesh(Mesh2D);
    ...
    test "faraday_step produces non-trivial update for non-zero E" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_FAD_01);
    ...
    test "faraday_step is B -= dt * d(E)" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_FAD_02);
        for (state.E.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
    ...
    test "ampere_step produces non-trivial update for non-zero B" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_A0B_01);
        for (state.B.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
    ...
    test "ampere_step is E += dt * (★₁⁻¹ d ★₂ B − J)" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_A0B_02);
        for (state.E.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
        for (state.B.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
    ...
    test "ampere_step with J source reduces E compared to without" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_A0B_03);
    ...
    test "leapfrog_step composes Faraday then Ampere" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_1F_01);
    ...
    test "leapfrog energy bounded over 100 source-free steps" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_1F_02);
    ...
    test "apply_pec_boundary is idempotent" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xBEC_01);
    ...
    test "Runner advances state by configured number of steps" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_4101);
        for (state.E.values) |*v| v.* = (rng.random().float(f64) * 2.0 - 1.0) * 0.01;
    ...
    test "electromagnetic_energy is non-negative" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0xE0_E40_01);
        for (state.E.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
        for (state.B.values) |*v| v.* = rng.random().float(f64) * 2.0 - 1.0;
    ...
    test "end-to-end: 100 steps, dB = 0 structurally, energy bounded" {
        ...
        for (0..num_steps) |step_idx| {
            ...
            comptime {
                local_std.debug.assert(MaxwellState.TwoForm.degree == Mesh2D.embedding_dimension);
            }

            // ── Invariant 2: energy is finite ──
            const energy = try electromagnetic_energy(allocator, &state);
            try testing.expect(!local_std.math.isNan(energy));
            try testing.expect(!local_std.math.isInf(energy));
    ...
    pub fn project_te10_e(mesh: *const Mesh2D, values: []f64, t: f64, domain_length: f64) void {
        ...
        const coords = mesh.vertices.slice().items(.coords);
        const k = local_std.math.pi / domain_length;
        const omega = k; // c = 1
    ...
    fn discrete_energy_from_arrays(allocator: local_std.mem.Allocator, mesh: *const Mesh2D, e_vals: []const f64, b_vals: []const f64) !f64 {
    ...
    fn run_cavity_energy_drift(allocator: local_std.mem.Allocator, grid_n: u32, final_time: f64) !f64 {
    ...
    fn compute_te10_eigenvalue(allocator: local_std.mem.Allocator, grid_n: u32, domain_length: f64) !f64 {
        var mesh = try Mesh2D.uniform_grid(allocator, grid_n, grid_n, domain_length, domain_length);
        defer mesh.deinit(allocator);
    ...
    test "TE₁₀ cavity: eigenvalue error reduces ≥3× when grid halves (O(h²) convergence)" {
        ...
        const allocator = testing.allocator;
        const domain_length: f64 = 1.0;
        const analytical_omega_sq = local_std.math.pi * local_std.math.pi / (domain_length * domain_length);
    ...
    test "MaxwellLeapfrog.step produces same result as leapfrog_step" {
        ...
        var rng = local_std.Random.DefaultPrng.init(0x62_1EAF);
...
const maxwell_3d = struct {
    ...
    pub const MaxwellState3D = StateForMesh(Mesh3D);

    fn tm110WaveNumbers(width: f64, height: f64) struct { kx: f64, ky: f64 } {
        return .{
            .kx = local_std.math.pi / width,
            .ky = local_std.math.pi / height,
        };
    }

    fn tm110AngularFrequency(width: f64, height: f64) f64 {
        const wave = tm110WaveNumbers(width, height);
        return local_std.math.sqrt(wave.kx * wave.kx + wave.ky * wave.ky);
    }

    pub fn faradayStep(allocator: local_std.mem.Allocator, state: anytype, dt: f64) !void {
        try core_mod.Core(@TypeOf(state.mesh.*), hooks(@TypeOf(state.mesh.*))).faradayStep(allocator, state, dt);
    }
    ...
    pub fn applyPecBoundary(_: local_std.mem.Allocator, state: anytype) !void {
        core_mod.Core(@TypeOf(state.mesh.*), hooks(@TypeOf(state.mesh.*))).applyPecBoundary(state);
    }

    pub fn leapfrogStep3D(allocator: local_std.mem.Allocator, state: anytype, dt: f64) !void {
        try core_mod.Core(@TypeOf(state.mesh.*), hooks(@TypeOf(state.mesh.*))).leapfrogStep(allocator, state, dt);
    }
    ...
    pub fn runDriver(allocator: local_std.mem.Allocator, config: Config3D) !void {
    ...
    pub fn makeCavityMesh(allocator: local_std.mem.Allocator, config: Config3D) !Mesh3D {
    ...
    fn seedClosedMagneticField(allocator: local_std.mem.Allocator, state: *MaxwellState3D) !void {
        var potential = try MaxwellState3D.OneForm.init(allocator, state.mesh);
        defer potential.deinit(allocator);

        var rng = local_std.Random.DefaultPrng.init(0x92_3D_B_00);
    ...
    pub fn divergenceNorm3D(allocator: local_std.mem.Allocator, state: *const MaxwellState3D) !f64 {
        ...
        return local_std.math.sqrt(divergence.norm_squared());
    }
    ...
    fn compute_tm110_eigenvalue(
        ...
        defer E.deinit(allocator);
        const omega = tm110AngularFrequency(width, height);
        project_tm110_e(&mesh, E.values, local_std.math.pi / (2.0 * omega), width, height);
    ...
    test "3D Maxwell snapshot exports projected E and B data on tetrahedral cells" {
        ...
        try testing.expect(local_std.mem.indexOf(u8, xml, "Name=\"E_intensity\"") != null);
        try testing.expect(local_std.mem.indexOf(u8, xml, "Name=\"B_flux\"") != null);
        try testing.expect(local_std.mem.indexOf(u8, xml, "NumberOfCells=\"6\"") != null);
    }
    ...
    test "project_tm110_e vanishes on non-z edges and x/y boundary z-edges" {
        ...
        project_tm110_e(&mesh, values, local_std.math.pi / (2.0 * omega), 1.0, 1.0);
    ...
    test "TM₁₁₀ cavity: eigenvalue error decreases under 3D refinement" {
        const allocator = testing.allocator;
        const analytical = 2.0 * local_std.math.pi * local_std.math.pi;
        ...
        local_std.debug.print("TM110 error: 2^3={d:.6}, 4^3={d:.6}, ratio={d:.2}\n", .{
            error_coarse, error_fine, error_coarse / error_fine,
        });
...
pub fn Config(comptime topological_dimension: u8) type {
    return switch (topological_dimension) {
        2 => maxwell_2d.Config2D,
        3 => maxwell_3d.Config3D,
        else => @compileError("Maxwell examples only support topological dimensions 2 and 3"),
    };
}
...
pub fn State(comptime topological_dimension: u8) type {
    return switch (topological_dimension) {
        2 => maxwell_2d.StateForMesh(Mesh(2)),
        3 => maxwell_3d.StateForMesh(Mesh(3)),
        else => @compileError("Maxwell examples only support topological dimensions 2 and 3"),
    };
}

pub fn runDriver(comptime topological_dimension: u8, allocator: root_std.mem.Allocator, config: Config(topological_dimension)) !void {
...
pub fn makeMesh(comptime topological_dimension: u8, allocator: root_std.mem.Allocator, config: Config(topological_dimension)) !Mesh(topological_dimension) {
    return switch (topological_dimension) {
...
pub fn leapfrogStep(comptime topological_dimension: u8, allocator: root_std.mem.Allocator, state: *State(topological_dimension), dt: f64) !void {
    switch (topological_dimension) {
        2 => try maxwell_2d.leapfrog_step(allocator, state, dt),
        3 => try maxwell_3d.leapfrogStep3D(allocator, state, dt),
        else => unreachable,
    }
}

pub fn seedReferenceMode(comptime topological_dimension: u8, allocator: root_std.mem.Allocator, state: *State(topological_dimension), dt: f64, width: f64, height: f64) !void {
    switch (topological_dimension) {
...
pub fn divergenceNorm(allocator: root_std.mem.Allocator, state: *const State(3)) !f64 {
    return maxwell_3d.divergenceNorm3D(allocator, state);
}
